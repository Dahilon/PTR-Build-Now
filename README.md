...
This is a closed-loop decision-support prototype, not only an analytics
dashboard:

`source evidence → validated signal → severity + confidence → constrained
response options → human approval → deployment record → outcome review`

- **Foundry** — the live build currently contains `Overdose Signal`,
  `Overdose Response Zone`, and `Overdose Response Deployment` object types,
  linked signal-to-zone and deployment-to-zone relationships, create/edit/
  delete actions, and the published **SF Overdose Response Operations**
  Workshop. The public dataset is mapped into `Overdose Signal`. The exact
  live state and next mappings are documented in `foundry/README.md`.
- **Python (`/scripts`)** — the reproducible evidence and decision-contract
  layer, config-driven via
  ...
  - `08_build_operational_workflow.py` — deterministic risk/confidence scores,
    three budget- and inventory-constrained response options, grounded AIP
    evidence packets, and a SHA-256 artifact manifest
  - `run_pipeline.py` — rebuilds simulated validation, real analysis, and
    operational outputs in one command without editing configuration
- **Frontend (`/frontend`)** — three interactive views: simulated method
  validation, real public-data findings, and an operator workflow that makes
  low-confidence evidence and human approval visible.

## Operational decision contract

The product layer intentionally separates concepts that are often collapsed:

- `risk_score` measures how unusual a reporting-period count is relative to
  preceding history. It is deterministic, bounded to 0–100, and never created
  by a language model.
- `confidence_score` measures whether the data supports a precise operational
  decision. Public citywide aggregates are capped at low confidence even when
  severity is high.
- Response scenarios are generated by exhaustive constrained search against
  a fixed budget and inventory. The inventory in this public demo is labeled
  `notional_demo`; no estimated reach or cost is presented as observed fact.
- Every scenario starts as `proposed` with `approval_required=true`. The local
  frontend can demonstrate routing, but only Foundry actions represent the
  governed approval lifecycle.
- AIP receives structured evidence packets with source files, mandatory
  caveats, and prohibited claims. Its job is explanation and missing-
  information triage—not numeric scoring or autonomous deployment.
...
libpysal 4.15). The machine used for final verification runs Python 3.14.6 in
...
The repository now includes the full reproducible pipeline, explicit data
contracts, regression/guardrail tests, artifact hashes, a three-view static
operator demo, and Foundry implementation specifications. The public-data
limitation remains real rather than engineered away: the next production data
milestone is governed access to exact EMS incident coordinates/timestamps,
verified site history, live resource inventory, and recorded outcomes.

See `foundry/README.md` for the ontology/action mapping and
`foundry/aip_prompt.md` for the evidence-grounded AIP contract.
